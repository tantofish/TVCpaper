...
There are many great works on head pose estimation, an the methods can be divided into several categories depending on the type of data required (i.e., 2D image or depth map). A comprehensive overview on methods required for 2D images is given by \cite{Ref_Survey}. The color image-based algorithms can be further divided into two types, feature-based \cite{Ref1,Ref8,Ref10,Ref11,Ref18} and appearance-based \cite{Ref2,Ref4,Ref5,Ref6,Ref7,Ref17} methods. Vatahska et al.[8] roughly classifies the pose into three different face orientation and then determine the head pose by the position of facial features. Yang et al \cite{Ref1} using vertical stereo camera to register face model which needs to mark feature points at start. Z. Li et al \cite{Ref23} using multi-class kernel to classify the face and pose estimation and it can achieved large rotation angle. Wu et al\cite{Ref25} proposed a two-stage approach to address the issue of data alignment and background noise when using subspace analysis. Storer et al \cite{Ref6} warp the input face to frontal and to estimate head pose by comparing the frontal and input face. Recently, Huang et al \cite{Ref23} can be easily robustified to deal with image noise using supervised local subspace learning, but it is not suit for real-time application. However, those methods relying on color image are sensitive to illumination variations, and therefore the accuracy will be affected under weak or no light environment.

With the development of fast depth map generating systems such as \cite{Ref12}, many works are inspired to use depth data as additional information to improve color-image based algorithms \cite{Ref1,Ref14,Ref20}, and several recent works use depth data as their primary information \cite{Ref3,Ref9,Ref16,Ref19}. Zhao et al \cite{Ref22} obtained 3D points by stereo camera, but these 3D points are less accuracy than which obtained by Kinect. Breitenstein et al. \cite{Ref3} proposed a system which can estimate large rotation angles, but GPU-based implement is required to achieve real-time performance.   On the other hand, the state of the art works \cite{Ref16,Ref19} require training procedure before using the system. We believe that a system with low-complexity and training-free is practical for users.
...
\bibitem{Ref23}
D. Huang, M. Storer, F. De la Torre, H. Bischof, “Supervised Local Subspace Learning for Continuous Head Pose Estimation,” IEEE Conference on Computer Vision and Pattern Recognition 2010

\bibitem{Ref24}
S. Z. Li, Qi. Fu, L. Gu, B. Schölkopf, Y. Cheng, H. Zhang, ”Kernel machine based learning for multi-view face detection and pose estimation,” Proceedings of Eighth IEEE International Conference on Computer Vision 2001

\bibitem{Ref25}
J. Wu and M. Trivedi, “A two-stage head pose estimation framework and evaluation,” Pattern Recognition, 41(3):1138–1158, 2008
